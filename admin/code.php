<?php
    ...
    if(isset($_POST['product_delete_btn'])){
        ...
        $post_id = $_POST['product_delete_btn'];

        $check_img_query = "SELECT * FROM productos WHERE id_productos = '$post_id' LIMIT 1";
        $img_res = mysqli_query($conn, $check_img_query);
        $res_data = mysqli_fetch_array($img_res);

        $image = $res_data['imagen'];

        $query = "DELETE FROM productos WHERE id_productos='$post_id' LIMIT 1";
    ...
    if(isset($_POST['product_add'])){
        ...
        $query = "INSERT INTO productos (id_categoria,nombre_producto,precio_unitario,sku,imagen,descripcion,autor,stock, slug) VALUES
                    ('$category_id','$name','$precio','$SKU','$filename','$description','$autor','$stock', '$slug')";
